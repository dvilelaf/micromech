...
def _extract_json(text: str) -> str:
    ...
    # Find JSON block containing "p_yes"
    match = _JSON_RE.search(text)
    if match:
        return match.group(0)

    # Find any parseable {...} block, prefer last one (usually the final answer)
...
def _validate_prediction(raw: str) -> str:
    ...
    try:
        ...
        logger.warning("Failed to parse superforcaster prediction JSON")
