...
class TestMechServerInit:
    ...
    def test_get_status_stopped(self, server_config: MicromechConfig):
        server = MechServer(server_config)
        server._load_tools()
    ...
    def test_get_status_running(self, server_config: MicromechConfig):
        server = MechServer(server_config)
...
class TestMechServerRecovery:
    ...
    @pytest.mark.asyncio
    async def test_recover_dedup(self, server_config: MicromechConfig):
        ...
        server = MechServer(server_config)
        server._queued_ids.add("r1")  # pretend already queued
        await server._recover()
...
class TestMechServerDedup:
    @pytest.mark.asyncio
    async def test_duplicate_request_skipped(self, server_config: MicromechConfig):
        server = MechServer(server_config)
    ...
    @pytest.mark.asyncio
    async def test_processed_request_skipped(self, server_config: MicromechConfig):
        ...
        await server._on_new_request(MechRequest(request_id="r1", prompt="dup"))
...
class TestMechServerProcessing:
    ...
    @pytest.mark.asyncio
    async def test_execute_and_cleanup_removes_from_dedup(self, server_config: MicromechConfig):
        server = MechServer(server_config)
        server._load_tools()
    ...
    @pytest.mark.asyncio
    async def test_stop_cancels_executor_tasks(self, server_config: MicromechConfig):
        server = MechServer(server_config)
        server._running = True
...
class TestMechServerRun:
    @pytest.mark.asyncio
    async def test_run_and_stop(self, server_config: MicromechConfig):
        """Server starts, processes a request, and stops gracefully."""
        server = MechServer(server_config)
        ...
        await asyncio.wait_for(server.run(with_http=False), timeout=5.0)

        record = server.queue.get_by_id("r1")
        assert record is not None
        # Should be executed (or at least attempted)
        assert record.request.status in ("executed", "failed")
        server.shutdown()

    @pytest.mark.asyncio
    async def test_run_loads_tools(self, server_config: MicromechConfig):
        server = MechServer(server_config)
